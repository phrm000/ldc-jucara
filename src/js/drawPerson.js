//Maps base points for each face element
let nosePoints = [
    [0.5, 0],
    [-5.5, 12],
    [0.5, 25],
...
function drawSplineWithBrush(avgX,avgY,shapeSize) {
    ...
    brush.set("cpencil", "#3B0B1A", shapeSize / 10);
    scaledPoints = eyebrowsPoints.map(([x, y]) => [x * shapeSize/70* randomCircleFactor(), y * shapeSize/70 * randomCircleFactor()]);
    brush.spline(scaledPoints, 0.8);
    pop();
    brush.pop();
